...
-   SACK 允许 TCP 单独确认非连续的片段，用来告知真正丢失的包，只重传丢失的片段。SACK 存储的是「一系列的非连续的没有确认数据的 seq range」，其中「seq range」内部当然连续，最多 4 组。SACK 用来存放收到但是没确认（没有发送对应 ACK）的包。
...
其中真正属于「接收窗口」的只有：**「未收到数据但可以接收的数据」**。但是缓冲区中有「未读取」+「可接收」两部分。
