...
## 第四讲 Loki 的 allocator

内存管理存在共性。

Loki Library 是业界一个很前沿的库，但是不是很成熟。Loki 的allocator 会把内存归还给 OS，在多进程的情况下更有利。

<img src="https://raw.githubusercontent.com/Missyesterday/picgo/main/picgo/image-20230818143014153.png" alt="image-20230818143014153" style="zoom:50%;" />

-   这三个类是相互包含的，上面的类有一个下面的类的`vector`，形成一个三层的结构，其中，每个类都有额外的两根指针，指向下一层某两个个位置。
-   我们一把只使用最上层的`SmallObjAllocator`类。
...
<img src="https://raw.githubusercontent.com/Missyesterday/picgo/main/picgo/image-20230818144317422.png" alt="image-20230818144317422" style="zoom:50%;" />

-   不会直接调用这个函数，而是被上层的函数调用。
-   `new`一块内存（大小是被指定的参数）

**Reset()函数：**

-   重新设置`Init()`，将`blocksAvaiable`设置为 64，`firstAvailableBlcok`设置为 0。
